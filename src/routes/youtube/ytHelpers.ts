...
import { tempFolderPath } from "./ytHandlers";
...
export const removeSessionFolder = (sessionID: string) => {
  const sessionFolder = path.resolve(tempFolderPath, sessionID);

  if (!fs.existsSync(sessionFolder)) {
    console.log("💀 session folder not exist");
    return;
  }
  ...
  fs.rm(sessionFolder, { recursive: true, force: true }, (err) => {
    if (err) {
      console.error(`🟥 Faild to delete the session folder -> ${err.message}`);
    } else {
...
export const downloadFile = (
  ...
  return new Promise((resolve) => {
    ...
    ytdl
      ...
      .on("error", (err) => {
        console.log(`🟥 ${err.message} - ytdl failed to download ${type} file`);
        failed = true;
